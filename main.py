...
def display_analysis_results(results):
    ...
    if action == "BELI/TAMBAH POSISI 🟢":
        ...
        print("  " + "*"*76)
        print("  *" + " "*74 + "*")
        print("  *" + " "*15 + "REKOMENDASI KUAT: BELI/TAMBAH POSISI 🟢" + " "*15 + "*")
        print("  *" + " "*74 + "*")
        print("  " + "*"*76)
        print("\n")
        print(f"Stop Loss       : Rp {format_number(recommendation['stop_loss'])}")
        ...
        print("  " + "*"*76)
        print("  *" + " "*74 + "*")
        print("  *" + " "*25 + "REKOMENDASI KUAT: JUAL 🔴" + " "*25 + "*")
        print("  *" + " "*74 + "*")
        print("  " + "*"*76)
        print("\n")
        if 'trailing_stop' in recommendation:
            print(f"Trailing Stop   : Rp {format_number(recommendation['trailing_stop'])} ({recommendation['trailing_stop_pct']:.1f}%)")
        ...
        print("  " + "*"*76)
        print("  *" + " "*74 + "*")
        print("  *" + " "*22 + "REKOMENDASI KUAT: SHORT SELL 🔴" + " "*22 + "*")
        print("  *" + " "*74 + "*")
        print("  " + "*"*76)
        print("\n")
        if 'trailing_stop' in recommendation:
        ...
        if 'target_price' in recommendation:
            print(f"Target Price    : Rp {format_number(recommendation['target_price'])}")
        print(f"Kondisi Pasar   : Bearish / Overbought")
        print(f"Tingkat Risiko  : {recommendation['risk_level']}")
    else:
        print("\n")
        print("  " + "-"*76)
        print("  |" + " "*74 + "|")
        print("  |" + " "*25 + "REKOMENDASI: TAHAN POSISI ⚪" + " "*25 + "|")
        print("  |" + " "*74 + "|")
        print("  " + "-"*76)
        print("\n")
        print(f"Level Support   : Rp {format_number(bb_lower)}")
    ...
    if action == "BELI/TAMBAH POSISI 🟢":
        ...
        conclusion = f"Berdasarkan analisis teknikal dan riwayat transaksi dengan keyakinan {confidence:.1f}%, "
        conclusion += "kondisi pasar mendukung untuk melakukan SHORT SELLING karena indikator teknikal "
        conclusion += "menunjukkan tren bearish yang kuat dan kondisi overbought. "
        if 'trailing_stop' in recommendation:
            conclusion += f"Gunakan trailing stop di level Rp {format_number(recommendation['trailing_stop'])} "
            conclusion += f"untuk membatasi risiko. "
    else:
        conclusion = f"Berdasarkan analisis teknikal dan riwayat transaksi dengan keyakinan {confidence:.1f}%, "
    ...
    volatility = data['Close'].pct_change().std() * 100  # Historical volatility as percentage
    ...
    explanation = generate_trade_explanation(
        ...
        technical_data={
            ...
            'bb_position': bb_position,
            'volume_ratio': volume / avg_volume if avg_volume > 0 else 1,
            'volatility': volatility  # Add volatility for dynamic thresholds
        },
        position_data=position_summary,
        recommendation=recommendation
    )
    
    # Print explanation with proper spacing
...
def generate_trade_explanation(action, technical_data, position_data, recommendation):
    ...
    """
    explanation = ""      # Profit/Loss explanation
    if position_data['current_position'] > 0:
        ...
        # Menggunakan volatilitas dan data teknical untuk menentukan batas dinamis
        # Drawdown threshold berdasarkan volatilitas (semakin volatil, semakin besar threshold)
        volatility = technical_data.get('volatility', 3.0)  # Default 3% jika tidak ada
        drawdown_threshold = max(5, min(volatility * 2, 15))  # Antara 5% dan 15%
        
        # Profit threshold berdasarkan RSI (semakin tinggi RSI, semakin rendah threshold)
        rsi = technical_data['rsi']
        profit_threshold = max(10, min(25 - (rsi - 50) * 0.3, 20))  # Antara 10% dan 20%
        
        if profit_loss_pct <= -drawdown_threshold:
            explanation += "💸 DRAWDOWN:\n"
            explanation += f"Anda telah mengalami kerugian lebih dari {drawdown_threshold:.1f}%. "
            if action == "JUAL 🔴":
        ...
        elif profit_loss_pct >= profit_threshold:
            explanation += "💰 PROFITABLE POSITION:\n"
            explanation += f"Anda telah mendapatkan keuntungan lebih dari {profit_threshold:.1f}%. Mengambil keuntungan (profit taking) adalah "
            explanation += "strategi yang baik untuk mengamankan keuntungan (lock in profit).\n"
            explanation += "Beberapa strategi yang umum: \n"
    ...
    if technical_data['macd_hist'] < 0:
        ...
        explanation += "Harga saham sedang dalam tren menurun (bearish), yang terlihat dari indikator MACD negatif "
        ...
        explanation += "MACD menunjukkan momentum bullish (naik), yang berarti ada potensi kenaikan harga dalam waktu dekat "
        explanation += "(uptrend). MACD positif menandakan bullish crossover, dimana rata-rata harga jangka pendek "
    ...
    if technical_data['volume_ratio'] > 1.3:
        ...
        explanation += "Volume perdagangan saat ini tinggi, yang menunjukkan adanya minat kuat dari pasar dan likuiditas yang baik. "
    ...
    rsi = technical_data['rsi']
    if rsi > 70:
        explanation += "⚠️ OVERBOUGHT CONDITION:\n"
        explanation += "RSI di atas 70 menunjukkan saham dalam kondisi jenuh beli, yang berarti harga mungkin "
        explanation += "akan mengalami koreksi dalam waktu dekat. Trader sering menggunakan RSI sebagai indikator "
        explanation += "'contrarian' - membuat keputusan berlawanan dengan arah pasar saat ini ketika mencapai ekstrim. "
        explanation += "Strategi 'overbought fade' melibatkan pengambilan posisi jual (short) ketika RSI menunjukkan "
        explanation += "jenuh beli.\n\n"
    elif rsi < 30:
        explanation += "💡 OVERSOLD CONDITION:\n"
        explanation += "RSI di bawah 30 menunjukkan saham dalam kondisi jenuh jual, yang berarti ada potensi "
        explanation += "pembalikan ke atas (rebound/reversal). Kondisi ini sering menjadi sinyal 'dip buying opportunity', "
        explanation += "terutama jika ditemukan pola 'bullish divergence' di mana harga membuat level terendah baru "
        explanation += "tetapi RSI tidak. Pada saham dengan fundamental kuat, ini sering menjadi entry point yang baik.\n\n"
    elif 40 <= rsi <= 60:
        explanation += "⚖️ NEUTRAL MOMENTUM:\n"
        explanation += "RSI berada pada zona netral (40-60), menunjukkan keseimbangan antara tekanan beli dan jual. "
        explanation += "Dalam kondisi ini, trader biasanya menunggu 'breakout confirmation' dari level support atau "
        explanation += "resistance.\n\n"
      # Final recommendation with professional terminology
    explanation += "✅ TRADE RECOMMENDATION:\n"
    
    if action == "BELI/TAMBAH POSISI 🟢":
        explanation += "LONG ENTRY SIGNAL\n"
        explanation += "- Beli/tambah posisi karena indikator menunjukkan bullish momentum dan potensi kenaikan harga\n"
        explanation += f"- Target profit dapat ditetapkan pada level resistance terdekat (Rp {format_number(recommendation['target_price'])})\n"
        explanation += f"- Stop loss di level Rp {format_number(recommendation['stop_loss'])} untuk manajemen risiko yang baik\n"
        explanation += f"- Risk-to-reward ratio {recommendation['risk_reward_ratio']:.2f}:1 disarankan untuk trade ini"
    elif action == "JUAL 🔴":
        explanation += "EXIT POSITION/SHORT ENTRY\n"
        explanation += "- Jual untuk mengamankan modal (capital preservation) dari potensi penurunan lebih lanjut\n"
        explanation += "- Berdasarkan bearish confirmation dari indikator teknikal\n"
        if 'trailing_stop' in recommendation:
            explanation += f"- Gunakan trailing stop {recommendation['trailing_stop_pct']:.1f}% (Rp {format_number(recommendation['trailing_stop'])}) untuk mengamankan sebagian keuntungan\n"
        explanation += "- Untuk trader agresif, sinyal ini juga bisa menjadi entry point untuk posisi short\n"
        explanation += "- Target pada level support terdekat dengan stop loss ketat di atas resistance terakhir"
    elif action == "SHORT SELL 🔴":
        explanation += "SHORT ENTRY SIGNAL\n" 
        explanation += "- Pertimbangkan short sell karena kondisi bearish yang kuat\n"
        explanation += "- Pastikan Anda memahami risiko short selling yang lebih tinggi\n"
        if 'trailing_stop' in recommendation:
            explanation += f"- Perhatikan trailing stop di level Rp {format_number(recommendation['trailing_stop'])} ({recommendation.get('trailing_stop_pct', 0):.1f}%)\n"
        if 'target_price' in recommendation:
            explanation += f"- Target price di level Rp {format_number(recommendation['target_price'])}\n"
        explanation += "- Strategi ini hanya untuk trader berpengalaman"
    else:
        explanation += "NEUTRAL STANCE\n"
        explanation += "- Tahan posisi (hold) dan pantau perkembangan chart pattern dan volume\n"
        explanation += "- Belum ada sinyal yang cukup kuat untuk mengambil tindakan agresif\n" 
        explanation += "- Perhatikan level support dan resistance terdekat untuk kemungkinan breakout\n"
        explanation += "- Strategi cash conservation dan wait-and-see sampai ada konfirmasi tren yang lebih jelas"
    ...
    try:
        while True:
            ...
            if choice == "1":
                ...
                stock_input = input("\nPilih nomor saham atau ketik -1 untuk input manual: ").strip()
                
                if not stock_input:  # Jika user hanya menekan Enter
                ...
                try:
                    idx = int(stock_input)
                    if idx == -1:
                        # Input manual (khusus untuk saham yang tidak ada di daftar)
            ...
            elif choice == "3":                # Add new transaction                clear_screen()
                print("\n===== Tambah Transaksi Baru =====")
                
                # Initialize transaction history
                th = TransactionHistory()
                # Get stock symbol                print("\nDaftar saham tersedia:")
                stocks = stock_config.get_stocks_without_suffix()
                for i, stock in enumerate(stocks, 1):
                    print(f"{i}. {stock}")
                ...
                tx_choice = input("\nPilih jenis transaksi: ").strip()

                if not tx_choice:  # Jika user hanya menekan Enter
                    # Kembali ke menu utama
                    continue

                try:
                    tx_type_choice = int(tx_choice)
                    if tx_type_choice == 0:
                        # Kembali ke menu utama
                        continue
                ...
                if not date_str:  # Jika user hanya menekan Enter
                    # Kembali ke menu utama
                    continue
                ...
                if not price_input:  # Jika user hanya menekan Enter
                    # Kembali ke menu utama
                    continue
                ...
                if not lot_input:  # Jika user hanya menekan Enter
                    # Kembali ke menu utama
                    continue
                ...
                try:
                    ...
                    # Tambahkan prompt untuk kembali ke menu utama
                    input("\nTekan Enter untuk kembali ke menu utama...")
                ...
                while True:
                    ...
                    if not sub_choice:  # Jika user hanya menekan Enter
                        break
                    elif sub_choice == "1":
                ...
                try:
                    ...
                    print("\nInformasi Dukungan:")
                    support_email = info.get('SUPPORT', 'SUPPORT_EMAIL', '-')
                    print(f"Email: {support_email}")
                    
                    print("\nMedia Sosial:")
                    social_media = info.get_social_media()
                    if social_media:
                    ...
                    if funding:
                        for platform, url in funding.items():
                            if url and not url.startswith('http'):
                                continue
                            print(f"- {platform.capitalize()}: {url}")
                except ImportError:
                    print("\nInformasi pengembang tidak tersedia.")
                except Exception as e:
                    print(f"\nError saat menampilkan informasi: {str(e)}")
